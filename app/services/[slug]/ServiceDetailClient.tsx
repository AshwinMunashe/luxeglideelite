...
import { LANG, SERVICES, PHONE, WHATSAPP } from "@/components/lib/Constants";
    ...
    return (
      <>
        ...
        <section className="page-hero">
          ...
          <Link href="/services" className="btn-o" style={{ display: "inline-flex", marginTop: 24 }}>
            {t.viewAll}
          </Link>
        </section>
        <Footer />
  ...
  return (
    <>
      ...
      <section className="page-hero">
        ...
        <h1 className={fd} style={{ fontSize: "clamp(32px,4.6vw,58px)", fontWeight: 400, fontStyle: lang === "ar" ? "normal" : "italic", color: "var(--off)", margin: "16px 0 14px" }}>{svc.title[lang]}</h1>
        <p className={fb} style={{ fontSize: 13, color: "var(--muted)", maxWidth: 560, margin: "0 auto", lineHeight: 1.8, fontWeight: 300 }}>{svc.desc[lang]}</p>
        <div className="breadcrumb" dir={t.dir}>
          <Link href="/">{lang === "en" ? "Home" : "الرئيسية"}</Link><span>/</span>
          <Link href="/services">{t.servicesTag}</Link><span>/</span>
          <span style={{ color: "var(--gold)" }}>{svc.title[lang]}</span>
        </div>
      </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 56, maxWidth: 1200, margin: "0 auto", alignItems: "start" }} className="svc-detail-grid">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} style={{ textAlign: isRTL ? "right" : "left" }}>
            ...
            <div style={{ borderInlineStart: "2px solid var(--gold)", paddingInlineStart: 16 }}>
              ...
              <p className={fb} style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.8, fontWeight: 300 }}>{svc.idealFor[lang]}</p>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ delay: .1 }}
            ...
            {svc.highlights[lang].map((h, i) => (
              <div key={i} className={fb} style={{
                display: "flex", alignItems: "flex-start", gap: 10, fontSize: 12.5, color: "var(--muted)",
                padding: "10px 0", borderBottom: i < svc.highlights[lang].length - 1 ? "1px solid rgba(255,255,255,.05)" : "none",
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              <a href={`https://wa.me/${WHATSAPP}?text=${encodeURIComponent(
                lang === "en" ? `Hi, I'd like to enquire about ${svc.title.en}.` : `مرحباً، أرغب بالاستفسار عن ${svc.title.ar}.`
      ...
      <section className="section-pad-b" style={{ padding: "0 clamp(24px,5vw,80px) 90px", background: "var(--black)" }} dir={t.dir}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div className={fb} style={{ fontSize: 10, letterSpacing: ".28em", textTransform: "uppercase", color: "var(--gold)", marginBottom: 20, textAlign: isRTL ? "right" : "left" }}>
            {lang === "en" ? "Other Services" : "خدمات أخرى"}
          </div>
